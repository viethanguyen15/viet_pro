    ...
    if(isset($_POST['sbm'])){
        ...
        if($num_row > 0){
            ...
            if($user_pass == $user_re_pass){
                $sql_add_user = "INSERT INTO user (user_full, user_mail, user_pass, user_level)
                                    VALUES ('$user_full', '$user_mail', '$user_pass', '$user_level')";
                $query_add_user = mysqli_query($conn, $sql_add_user);
                header('location: index.php?page_layout=user');                    
            }
            else{
                $error_pass = '<div class="alert alert-warning">Nhập đúng mật khẩu tạo và nhập lại</div>';
            }
        }
    }
?>		
